...
const ActionItem: React.FC<ActionItemProps> = (props) => {
    ...
    useEffect(() => {
        if(context?.inputState){
            const { bill: contextBill, person: contextPerson, tip: contextTip } = context.inputState;
        ...
        return(
            <div>
                <div className="d-flex justify-content-between">
                    ...
                    {inputBillVal === 0 && (
                        <label htmlFor="" className={styles.formLabelError}>Can't be zero</label>
                    )}
                </div>
                <div className={`input-group ${inputBillVal === 0 ? styles.error : ''}`} data-id="bill">
                    <span className={`input-group-text ${styles.inputIcon} ${styles.success}`}>
                        <img src={DollarIconImg} alt={forType} height="20px" width="auto" />
                    </span>
                    <input type="number" value={inputBillVal} onChange={handleBillChange} className="form-control fs-5" id="bill" name="bill" aria-label="bill" />
                </div>
            </div>
        )
    }
        ...
        return(
            <div>
                <div className="d-flex justify-content-between">
                    ...
                    {inputPersonVal === 0 && (
                        <label htmlFor="people" className={styles.formLabelError}> Can't be zero </label>
                    )}
                </div>
                <div className={`input-group ${inputPersonVal === 0 ? styles.error : ''}`} data-id="person">
                    <span className={`${styles.inputIcon} input-group-text`}>
                        <img src={PersonIconImg} alt={forType} height="20px" width="auto" />
                    </span>
                    <input type="number" value={inputPersonVal} onChange={handlePersonChange} className="form-control fs-5" id="people" name="people" aria-label="people" />
                </div>
            </div>
        )
    }
    ...
    if (forType === 'tipInput'){
        return (
            <div className="container px-0">
                <div>
                    <div className="col px-0"> <label > Select Tip % </label> </div>
                </div>
                <div className={`${styles.tipGrid} mt-2`}>
                    {TIP_VALS.map((tip) => {
                        return(
                            <button key={`${tip}%`} type="button" onClick={handleTip} value={tip} className={`${styles.tipBtn} ${tip === inputState.tip ? styles.tipActive : ''}`} name="tip-val">
                                {tip}%
                            </button>
                        )
                    })}
                    <input type="number" value={inputTipVal} onChange={handleTipInput} placeholder="Custom" className="form-control fs-5" id="tip" name="tip" aria-label="tip" />
                </div>
            </div>
        )
    }
