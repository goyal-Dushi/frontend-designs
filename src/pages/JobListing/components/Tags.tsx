...
const Tags: React.FC<TagProps> = (props) => {
    ...
    if(newTag){
        return(
            <span className={`${styles.new_tag} badge rounded-pill`}>
                NEW!
            </span>
        )
    }
            ...
            {deleteArr.map((names, i) => {
                return(
                    <div key={names+'-'+i} className={'me-2'} >
                        <div className={`${styles.deletable_tag} badge`}>
                            {names}
                        </div>
                        <button onClick={() => handleDelete(names)} className={styles.delete_icon} > X </button>
                    </div>
                )
            })}
            </>
        ...
        return (
            <>
                {languages.map((language, i) =>
                    <div key={i+'-'+language} className={`${styles.skill_tag} badge mx-2 my-1`}>
                        {language}
                    </div>
                )}
            </>
